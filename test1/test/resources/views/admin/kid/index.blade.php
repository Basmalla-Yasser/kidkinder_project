        ...
        <div class="content-wrapper">
            <div class="row">
                <div class="col-lg-12 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">Kid</h4>
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <!-- Left aligned Add button and search -->
                                <a href="{{ route('kid.create') }}" class="btn btn-primary ms-3">
                                    Add
                            ...
                            <table id="myTable" class="table table-striped table-bordered text-center mx-3">
                                ...
                                <tbody>
                                    @foreach ($kids as $kid)
                                        <tr>
                                            <td><strong>{{ $kid->id }}</strong></td>
                                            <td><strong>{{ $kid->name}}</strong></td>
                                            <td><strong>{{ $kid->age}}</strong></td>
                                            <td><strong>{{ $kid->address}}</strong></td>
                                            <td><strong>{{ $kid->hobby}}</strong></td>

                                            <td><strong>{{ $kid->father->name }}</strong></td>


                                            <td><strong>

                                                <a href="{{ route('kid.edit', $kid->id) }}"
                                                ...
                                                <a href="javascript:;" class="btn btn-primary btn-sm p-2 mx-2"
                                                    ...
                                                    data-bs-target="#deleteSliderModal{{ $kid->id }}">
