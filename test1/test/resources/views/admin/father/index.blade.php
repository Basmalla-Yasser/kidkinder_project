        ...
        <div class="content-wrapper">
            <div class="row">
                <div class="col-lg-12 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">Fathers</h4>
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <!-- Left aligned Add button and search -->
                                <a href="{{ route('father.create') }}" class="btn btn-primary ms-3">
                                    Add
                                ...
                                <table id="myTable" class="table table-striped table-bordered text-center mx-3">
                                    ...
                                    <tbody>
                                        @foreach ($fathers as $father)
                                            <tr>
                                                <td><strong>{{ $father->id }}</strong></td>
                                                <td><strong>{{ $father->address }}</strong></td>
                                                <td><strong>{{ $father->email }}</strong></td>
                                                <td><strong>{{ $father->phone }}</strong></td>
                                                <td><strong>{{ $father->state }}</strong></td>
                                                <td><strong>{{ $father->job }}</strong></td>
                                                <td><strong>{{ $father->name }}</strong></td>
                                                <td>
                                                    <a href="{{ route('father.edit', $father->id) }}"
                                                    ...
                                                    <a href="javascript:;" class="btn btn-primary btn-sm p-2 mx-2"
                                                        ...
                                                        data-bs-target="#deleteSliderModal{{ $father->id }}">
